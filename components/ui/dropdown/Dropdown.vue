<template>
    <div class="w-auto h-auto relative">
        <div class="w-auto h-auto py-2 px-4 border rounded-md flex justify-between items-center gap-4 z-20" @click="toggleDropdown">
            {{ title ? title : activeValue.title }}
            <IconCSS name="mdi:chevron-down" class="text-[#999999] text-lg"/>
        </div>
        <Transition name="fade">
            <ul class="dropdown-body w-full h-auto py-2 px-4 border rounded-md absolute top-[110%] left-0 bg-white z-10 overflow-hidden" v-if="toggle">
                <li 
